...
import { propertyCreateSchema } from '@/validators/propertySchema';
...
import { 
  Building, MapPin, BadgeIndianRupee, Bed, HelpCircle, Image, Eye, CheckCircle2, 
...
const CreateProperty = () => {
  ...
  const handleMediaUpload = async (e) => {
    ...
    try {
      ...
      // Upload files sequentially or mapped
      const uploadedImages = [...images];
      for (const file of files) {
        const formData = new FormData();
        formData.append('image', file);
        
        // Call uploader service
        const data = await userService.uploadProfilePicture(formData);
        
        uploadedImages.push({
          publicId: `upload_${Date.now()}`,
          url: data.avatar, // Reuse avatar upload response which returns local/cloudinary URL
  ...
  const handleRemoveImage = (idx) => {
    ...
    // Reassign primary if removed primary
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 md:px-6 py-6 space-y-6">
      ...
      {/* 2. Step Progress Indicators (Section 3) */}
        ...
        {STEPS.map((step, idx) => {
          ...
          return (
            <div key={step.label} className="flex items-center space-x-2 shrink-0">
              <div className={`p-2 rounded-full text-xs font-bold transition-all ${isActive ? 'bg-primary-500 text-white scale-102' : isCompleted ? 'bg-success-100 text-success-700' : 'bg-secondary-200 text-secondary-550'}`}>
      ...
      <Card className="p-6 border-secondary-200/50 space-y-6">
        <FormProvider {...methods}>
          <form onSubmit={handleSubmit(onSubmitListing)} className="space-y-6">
            ...
            {/* Step 6: Images System (Section 4) */}
            {activeStep === 5 && (
            ...
            {/* Step 7: Preview details before submission */}
            {activeStep === 6 && (
              <div className="space-y-6">
                <div className="border-b border-secondary-100 dark:border-secondary-900 pb-2">
